...
function getFirstChar(val) {
  return val.substr(0, val.length - val.length + 1);
}
...
function extractEmails(str) {
  const arr = str.match(/([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9._-]+)/gi);
  return arr;
}
...
function encodeToRot13(str) {
  ...
  const index = (x) => inp.indexOf(x);
  const trans = (x) => (index(x) > -1 ? outp[index(x)] : x);
  const arr = str.split('').map(trans).join('');
  return arr;
}
...
function isString(val) {
  ...
  if (val instanceof String || str === 'string') {
    return true;
  }
  return false;
}
