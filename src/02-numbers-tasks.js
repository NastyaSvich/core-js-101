...
function getParallelipidedDiagonal(a, b, c) {
  const itog = Math.sqrt(a * a + b * b + c * c);
  return itog;
}
...
function roundToPowerOfTen(num, pow) {
  ...
  if (pow === 1) {
    const fir = num / 10;
    const okr = Math.round(fir);
    const itog = okr * 10;
    return itog;
  }
  if (pow === 2) {
    const fir1 = num / 100;
    const okr1 = Math.round(fir1);
    const itog1 = okr1 * 100;
    return itog1;
  }
  if (pow === 3) {
    const fir2 = num / 1000;
    const okr2 = Math.round(fir2);
    const itog2 = okr2 * 1000;
    return itog2;
  }
  return num;
}
